...
public class httpHelper {
	...
	public String getLiveAircraftType() {
		try {
			...
			parser.indicators ind = new parser.indicators();
			ind.init();
			ind.update(indicatorsJson);

			if (ind.valid != null && ind.valid.equals("true") && ind.type != null && !ind.type.isEmpty()
					&& !ind.type.equals("No Cockpit")) {
				return ind.type.toLowerCase().trim();
			}
		} catch (Exception e) {
			// e.printStackTrace();
			// 忽略错误，返回 null
